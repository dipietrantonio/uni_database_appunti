...
$Z^{(1)}= ABCD$;\\
Entriamo nel for:\\
\indent $S^{(2)} = S^{(1)} \cup [(ABCD \cap ABCD)^+_F \cap ABCD] = ABCD$;\\
\indent $S^{(3)} = S^{(2)} \cup [(ABCD \cap CDEH)^+_F \cap CDEH] = ABCDEH$;\\

\noindent Terminato il ciclo for notiamo che un'altra volta $S\not\subseteq Z$, quindi rieseguiamo il ciclo while:\\\\
$Z^{(2)}= ABCDEH$;\\
entriamo nel for:\\
\indent $S^{(4)} = S^{(3)} \cup [(ABCDEH \cap ABCD)^+_F \cap ABCD] = ABCD$;\\
...
esce dal ciclo \textsc{while} per tornare all'Algoritmo 1. $CD \subseteq ABCDEH$ 
quindi la variabile \emph{successo} rimane uguale a \emph{true}; per ogni altra dipendenza seguiamo lo
stesso procedimento. Troveremo quindi che: $AB\rightarrow CD \in G^+$, $E \rightarrow H  \in G^+$, 
